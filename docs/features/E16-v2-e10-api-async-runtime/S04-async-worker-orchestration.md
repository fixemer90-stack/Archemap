...
Run long generation via worker/queue with reliable retries and status persistence.
...
```bash
cd backend && uv run pytest tests/unit/test_astrotype_v2/test_api_runtime.py::test_worker_task_is_registered_but_runtime_module_does_not_import_legacy_pipeline -v --tb=short
```
