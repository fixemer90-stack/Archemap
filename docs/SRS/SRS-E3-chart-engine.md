...
Документ описывает программные требования к модулю **Profile & Chart Engine** — вычислительному ядру платформы Astrotype. Модуль отвечает за хранение натальных данных пользователя, вычисление астрологической карты и извлечение нормализованных признаков для последующей интерпретации rule engine.
...
|---|---|
| **Astrotype Self** | Натальная карта + feature vector → интерпретация архетипа |
| **Astrotype Love** | Две карты → синастрия, совместимость |
| **Astrotype Child** | Карта ребёнка → профиль темперамента |
| **Astrotype Career** | Карта → карьерные сильные стороны |
...
| C4 Architecture | `docs/C4 архитектура SaaS-платформы Astrotype.md` |
