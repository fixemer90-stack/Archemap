"""Migration contract tests for Astrotype v2 report artifact storage."""
...
def _artifact_migration_text() -> str:
    migration_files = sorted((ROOT / "alembic" / "versions").glob("*_add_astrotype_v2_report_artifacts.py"))

    assert len(migration_files) == 1
    return migration_files[0].read_text()


def test_report_artifact_migration_creates_only_v2_artifact_tables() -> None:
    migration_text = _artifact_migration_text()
...
def test_report_artifact_migration_downgrade_drops_only_v2_artifact_tables() -> None:
    migration_text = _artifact_migration_text()
