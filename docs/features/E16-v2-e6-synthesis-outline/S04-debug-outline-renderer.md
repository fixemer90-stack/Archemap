# V2-E6 S04: Add debug deterministic outline view
...
Render outline and facts without LLM for inspection and tests.
...
```bash
cd backend && uv run pytest tests/unit/test_astrotype_v2/test_outline.py::test_render_debug_outline_payload_is_deterministic_and_contains_no_llm_fields -v --tb=short
```
