...
```
FeatureVector (E3)          E4                        E5
┌─────────────────┐    ┌──────────────┐    ┌─────────────────┐
│ earth: 0.57     │    │ Rule Engine  │    │ Jinja2 Template │
│ fixed: 0.40     │───►│ Scoring      │───►│ HTML / PDF      │
│ house_5: 1.0    │    │ Claims       │    │ Report          │
│ sun: Virgo      │    │ Confidence   │    │                 │
└─────────────────┘    └──────────────┘    └─────────────────┘
```
...
FeatureVector (E3)
    ...
    ▼
┌─────────────────────┐
│ Load RuleSetVersion  │  ← YAML из PostgreSQL/Redis
│ (product + version)  │
└─────────┬───────────┘
          │
          ▼
┌─────────────────────┐
│ Evaluate Conditions  │  ← для каждого правила:
│ (all/any/not)        │    match_r = 0.0-1.0
└─────────┬───────────┘
          │
          ▼
┌─────────────────────┐
│ Calculate            │  ← contrib_r = w_r × match_r × q_input × q_rule
│ Contributions        │
└─────────┬───────────┘
          │
          ▼
┌─────────────────────┐
│ Aggregate Scores     │  ← score_k = clamp((bias + Σ support - λ×Σ counter) / max, 0, 1)
│ (archetypes/claims)  │
└─────────┬───────────┘
          │
          ▼
┌─────────────────────┐
│ Compute Confidence   │  ← 4-факторная модель
│ + Evidence Trail     │
└─────────┬───────────┘
          │
          ▼
┌─────────────────────┐
│ Select & Emit Claims │  ← threshold, diversity caps
│ (explainable output) │
└─────────────────────┘
```
