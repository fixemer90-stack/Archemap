# V2-E4 S04: Build reference lookup service
...
Create helpers that resolve calculated aspects to active versioned interpretations.
...
- [x] Calculated aspects resolve through a thin lookup service over repository reference methods.
- [x] Reversed body pairs are canonicalized before lookup and do not require duplicate rows.
- [x] Missing reference rows return `None` without legacy fallback.
...
```bash
cd backend && uv run pytest tests/unit/test_astrotype_v2/test_reference_lookup.py tests/unit/test_astrotype_v2/test_reference_planet_order.py tests/unit/test_astrotype_v2/test_reference_pair_data.py -q
