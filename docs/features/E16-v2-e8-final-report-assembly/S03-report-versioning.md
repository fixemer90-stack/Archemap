# V2-E8 S03: Persist report versions
...
Save final report artifacts with contract version, input hashes and regeneration lineage.
...
```bash
cd backend && uv run pytest tests/unit/test_astrotype_v2/test_report_assembler.py::test_build_natal_report_row_versions_without_overwriting_prior_artifacts -v --tb=short
```
